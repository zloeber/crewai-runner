...
def api_server():
    ...
    process = subprocess.Popen(
        ["/Users/zacharyloeber/dyad-apps/crewai-runner/.venv/bin/python", str(server_script)],
        cwd=str(api_dir),
...
def test_api_with_auth_header(api_server):
    ...
    try:
        response = requests.get(f"{api_server}/api/workflows/frameworks", headers=headers, timeout=10)
