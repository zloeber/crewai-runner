...
def mock_dependencies():
    ...
    with patch("engine.services.orchestrator_factory.OrchestratorFactory") as mock_factory:
        mock_orchestrator = Mock()
        mock_orchestrator.execute.return_value = {"workflow_id": "test-123", "status": "started"}
...
def api_server_check():
    ...
    try:
        response = requests.get(f"{TEST_API_BASE_URL.replace('/api', '')}/api/docs", timeout=5)
        if response.status_code == 200:
            return TEST_API_BASE_URL
        else:
            pytest.skip("API server is not responding correctly")
    except requests.exceptions.RequestException:
        pytest.skip("API server is not running. Start with: cd api && ENGINE_API_KEY=test-api-key python run_server.py")
