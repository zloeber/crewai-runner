...
def mock_providers_db():
    ...
    with patch("routers.providers.providers_db") as mock_db:
        ...
        mock_db.__getitem__.side_effect = lambda key: test_provider if key == "test-provider-123" else None
...
@pytest.mark.asyncio
async def test_list_providers_with_data(api_client, mock_providers_db, skip_if_no_server):
...
@pytest.mark.asyncio
async def test_add_provider_success(api_client, sample_provider_config, skip_if_no_server):
...
@pytest.mark.asyncio
async def test_add_provider_already_exists(api_client, sample_provider_config, skip_if_no_server):
...
@pytest.mark.asyncio
async def test_add_provider_with_existing_id(api_client, sample_provider_config, skip_if_no_server):
