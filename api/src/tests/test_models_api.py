...
def mock_models_db():
    ...
    with patch("routers.models.models_db") as mock_db:
        ...
        mock_db.__getitem__.side_effect = lambda key: test_model if key == "test-model-123" else None
...
@pytest.mark.asyncio
async def test_list_models_with_data(api_client, mock_models_db, skip_if_no_server):
    ...
    assert data["models"][0]["modelName"] == "gpt-4"
...
async def test_add_model_success(api_client, sample_model_config, skip_if_no_server):
    ...
    with patch("routers.models.models_db") as mock_db:
        mock_db.__contains__.return_value = False  # Model doesn't exist
        ...
        assert data["model"]["modelName"] == "gpt-4"
...
@pytest.mark.asyncio
async def test_add_model_already_exists(api_client, sample_model_config, skip_if_no_server):
...
@pytest.mark.asyncio
async def test_add_model_with_existing_id(api_client, sample_model_config, skip_if_no_server):
